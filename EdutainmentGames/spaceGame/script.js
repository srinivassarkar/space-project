    ...
    function fire() {
        ...
        for(const element of bullets) {
            if(!element.destroyed) {
                ...
                //Real coords
                element.realX = (0) - (element.y + 10) * Math.sin(element.deg);
                element.realY = (0) + (element.y + 10) * Math.cos(element.deg);
                ...
                for(var j = 0; j < asteroids.length; j++) {
                    if(!asteroids[j].destroyed) {
                        distance = Math.sqrt(Math.pow(
                                asteroids[j].realX - element.realX, 2) +
                            Math.pow(asteroids[j].realY - element.realY, 2)
                        );
